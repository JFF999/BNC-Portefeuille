...
def charger_donnees_base(nom_feuille):
    ...
    df = pd.read_excel("Action_2026-c.xlsx", sheet_name=nom_feuille, engine='openpyxl')
    # NOUVEAU : Nettoyage automatique des sauts de ligne (Alt+Enter) dans les noms de colonnes Excel
...
try:
    with st.spinner("Connexion à Yahoo Finance..."):
        ...
        # Sécurité : Essaie 'Prospects' avec un S, sinon rabat sur 'Prospect' sans S
        try:
    ...
    with tab1:
        ...
        st.dataframe(
            ...
            # NOUVEAU : Ajout de "Pré 1an $ Affaires" dans l'ordre des colonnes juste après "Pré 1an $"
            ...
            column_config={
                ...
                "Pré 1an $": st.column_config.NumberColumn("Pré 1an $ YF", format="$ %.2f"), # Précisé "YF" pour faire la distinction
                "Pré 1an $ Affaires": st.column_config.NumberColumn("Pré 1an $ Aff.", format="$ %.2f"), # Nouvelle colonne
    ...
    def surligner_prospects(row):
        if row.get('Possede') == True:
            return ['background-color: rgba(255, 215, 0, 0.5)'] * len(row)
        return [''] * len(row)
